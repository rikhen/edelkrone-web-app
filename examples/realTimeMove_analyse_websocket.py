...
import websocket # python3 -m pip install websocket-client

# Local SDK endpoint URL
wsURL = "ws://127.0.0.1:32224/v1"
...
# ---------------------------------------------------------------------------
def bundleStatus(socket):
  payload = {}
  payload = json.dumps(payload)
  socket.send(payload)
  response_json = json.loads(socket.recv())
  return response_json

# ---------------------------------------------------------------------------
def realTimeMoveFixedDuration(
  socket, duration, pan, tilt, slide, focus, debug=False
):
...
if __name__ == "__main__":
  ...
  ws_status.connect(wsURL + "/bundle/" + linkID + "/status")

  # ...
  ws_cmd = websocket.WebSocket()
  ws_cmd.connect(wsURL + "/bundle/" + linkID)  
